<template>
    <div>
        <div class="select-title-btn">
            <el-button class="add-btn" type="primary" @click="openAdd"><svg-icon icon-class="edit" />创建优惠券</el-button>
        </div>
        ...
        <el-table :data="tableData" border class="el-table">
            ...
            <el-table-column align="center" label="优惠券ID" width="100">
                <div slot-scope="scope">
                    {{ scope.row.id }}
                </div>
            </el-table-column>

            <el-table-column align="center" label="优惠券面值" width="100">
                <div slot-scope="scope">
                    {{ scope.row.price }}
                </div>
            </el-table-column>
            ...
            <el-table-column align="center" label="使用期限" width="180">
                <div slot-scope="scope">
                    {{ scope.row.start_time }} 至 {{ scope.row.end_time }}
                </div>
            </el-table-column>
            
            <el-table-column align="center" label="发行量" width="100">
                <template slot-scope="scope">
                    {{ scope.row.c_num }}
                </template>
            </el-table-column>

            <el-table-column  align="center" label="已领取" width="100">
                <template slot-scope="scope">
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :model="rulesForm" :rules="rules" style="width: 600px; margin-left: 50px" >
                ...
                <el-form-item label="类型" prop="type" label-width="96px">
                ...
                <el-form-item label="面值" prop="price" label-width="96px">
                    <el-input-number v-model="rulesForm.price" :min="1" :step="1" :controls="true"></el-input-number>
                    <span>元</span>
                </el-form-item>

                <el-form-item label="发行量" prop="s_num" label-width="96px">
                    <el-input-number v-model="rulesForm.c_num" :min="100" :step="1" :controls="true"></el-input-number>
                    <span>张</span>
                </el-form-item>

                <el-form-item label="已领取" prop="s_num" label-width="96px">
                    <el-input-number v-model="rulesForm.received_num" :min="1" :step="1" :controls="true"></el-input-number>
                    <span>张</span>
                </el-form-item>

                <el-form-item label="已使用" prop="s_num" label-width="96px">
                    <el-input-number v-model="rulesForm.used_num" :min="1" :step="1" :controls="true"></el-input-number>
                    <span>张</span>
                </el-form-item>

                <el-form-item label="秒杀活动时间">
                     <el-date-picker @change="seleteDate" v-model="timerange" type="daterange" start-placeholder="开始日期" end-placeholder="结束日期" ></el-date-picker>
                </el-form-item>
...
export default {
    ...
    methods: {
        // 获取列表
        async getList(){
            await fetchCoupon(this.listQuery).then((res) => {
                // console.log(res)
                if (res.code === 20000) {
        ...
        createData() {
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    createCoupon(this.rulesForm).then(() => {
                        this.dialogFormVisible = false
                        this.$notify({
                            title: 'Success',
        ...
        // 处理时间
        seleteDate(start_time,end_time){
            this.rulesForm.start_time = Monent(start_time).format('yyyy-mm-dd HH:mm:ss')
            this.rulesForm.end_time = Monent(end_time).format('yyyy-mm-dd HH:mm:ss')
            console.log(this.rulesForm.start_time,this.rulesForm.end_time );
        }
    }
}
</script>
