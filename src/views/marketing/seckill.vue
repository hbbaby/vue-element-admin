<template>
    <div>
        <div class="select-title-btn">
            <el-button class="add-btn" type="primary" @click="openAdd"><svg-icon icon-class="edit" />创建秒杀</el-button>
        </div>
        ...
        <el-table :data="tableData" border class="el-table">
            ...
            <el-table-column align="center" label="限制人数" width="120">
                <template slot-scope="scope">
                    {{ scope.row.s_num }}
                </template>
            </el-table-column>
            <el-table-column  align="center" label="已使用人数" width="160">
                <template slot-scope="scope">
                    {{ scope.row.used_num }}
                </template>
            </el-table-column>
            <el-table-column align="center" label="秒杀状态" width="120">
                <template slot-scope="scope">
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :model="rulesForm" :rules="rules" style="width: 600px; margin-left: 50px" >
                ...
                <el-form-item label="秒杀价" prop="price" label-width="96px">
                    <el-input-number v-model="rulesForm.price" :min="1" :step="1" :controls="true"></el-input-number>
                </el-form-item>

                <el-form-item label="秒杀人数" prop="s_num" label-width="96px">
                    <el-input-number v-model="rulesForm.s_num" :min="1" :step="1" :controls="true"></el-input-number>
                </el-form-item>

                <el-form-item label="已使用人数" prop="s_num" label-width="96px">
...
<script>
import { fetchFlashsale,createFlashsale,updateFlashsale } from '../../api/marketing'
...
export default {
    ...
    methods: {
        // 获取列表
        async getList(){
            await fetchFlashsale(this.listQuery).then((res) => {
                // console.log(res)
                if (res.code === 20000) {
        ...
        createData() {
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    createFlashsale(this.rulesForm).then(() => {
                        this.dialogFormVisible = false
                        this.$notify({
                            title: 'Success',
